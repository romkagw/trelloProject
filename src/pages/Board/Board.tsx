			...
			<div className='board-content'>
				{state.lists &&
					state.lists.map((el, index) => (
						<List id={el.id} title={el.title} cards={el.cards} position={index} key={el.id} />
					))}
				{isShow ? (
					<div ref={ref}>
						<Modal backDrop={false} setActive={setIsShow} width={'277px'} height={'85px'}>
							<ModalCreateList onClose={() => setIsShow(false)} board_id={board_id} />
						</Modal>
					</div>
				) : (
